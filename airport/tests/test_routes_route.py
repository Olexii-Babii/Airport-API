...
class AuthenticatedUserRouteTestCase(TestCase):
    ...
    def test_authenticated_route_list(self):
        sample_route(
            distance=500,
            source=sample_airport(name="test airport 1", closest_big_city="big city 1"),
            destination=sample_airport(
                name="test airport 2", closest_big_city="big city 2"
            ),
        )

        sample_route(
            distance=250,
            source=sample_airport(name="test airport 3", closest_big_city="big city 3"),
            destination=sample_airport(
                name="test airport 4", closest_big_city="big city 4"
            ),
        )
        response = self.client.get(ROUTE_URL)
