...
class UnauthenticatedUserFlightTestCase(TestCase):
    ...
    def test_unauthenticated_update_flight(self):
        ...
        response = self.client.patch(reverse("airport:flight-detail", kwargs={"pk": 1}), data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_unauthenticated_delete_flight(self):
        response = self.client.delete(reverse("airport:flight-detail", kwargs={"pk": 1}))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class AuthenticatedUserFlightTestCase(TestCase):
    ...
    def test_authenticated_flight_list(self):
        sample_flight(
            airplane_type="fight airplane type 2"
        )

        sample_flight(
            airplane_type="flight airplane type 3"
        )
        response = self.client.get(FLIGHT_URL)
        serializer = FlightListSerializer(Flight.objects.annotate(
                available_seats=(F("airplane__rows") * F("airplane__seats_in_row")) - Count("tickets"))
                                          .order_by("id"),
                                          many=True)
...
class FilterFlightQuerysetTestCase(TestCase):
    ...
    def test_correct_filter_by_source(self):
        ...
        serializer = FlightListSerializer(flight_queryset().filter(
            Q(route__source__name__icontains="bar") |
            Q(route__source__closest_big_city__icontains="bar")), many=True)
    ...
    def test_correct_filter_by_destination(self):
        ...
        serializer = FlightListSerializer(flight_queryset().filter(
            ...
            Q(route__destination__closest_big_city__icontains="ber")), many=True)
    ...
    def test_correct_filter_by_departure_time(self):
        ...
        serializer = FlightListSerializer(flight_queryset().filter(departure_time__date="2024-12-30"), many=True)
    ...
    def test_correct_filter_by_arrival_time(self):
        ...
        serializer = FlightListSerializer(flight_queryset().filter(arrival_time__date="2024-11-30"), many=True)
    ...
    def test_return_zero_flights(self):
        ...
        serializer = FlightListSerializer(Flight.objects.filter(
            Q(route__source__name__icontains="test1234") |
            Q(route__source__closest_big_city__icontains="test1234")), many=True)
